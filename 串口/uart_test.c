...
int set_opt(int fd,int nSpeed, int nBits, char nEvent, int nStop)
{
	...
	newtio.c_cc[VTIME]  = 1; //VTIME:非cannoical模式读时的延时，以十分之一秒位单位
	newtio.c_cc[VMIN] = 255; //VMIN:非canonical模式读到最小字符数
...
int main(int argc, char *argv[])
{
	...
	{   
		...
		ret = write(fd1, buf, 60);  
		if( ret > 0){  
			printf("write success!  wait data receive\n");  
		}  
		//sleep(3);  
		nread = read(fd1, buf1, 255);  
		if(nread > 0){  
